// Icon by name. Names keep the `lucide:<kebab-name>` form used throughout the
// content files (profile.ts, nav.ts), resolved here to lucide-react components.
// Only the icons the site uses are registered, so the bundle stays tree-shaken.
// GitHub and LinkedIn are no longer shipped by Lucide; they are drawn inline in
// the same stroke style so they sit next to the others without a visible seam.
import type { ComponentType, ReactNode, SVGProps } from 'react';
import {
  ArrowRight,
  AtSign,
  BotMessageSquare,
  Brain,
  Briefcase,
  Check,
  ChevronDown,
  Clock,
  Cloud,
  CodeXml,
  Download,
  FileText,
  Gamepad2,
  Gauge,
  Globe,
  GraduationCap,
  Hammer,
  Handshake,
  Layers,
  Lightbulb,
  Mail,
  MapPin,
  Menu,
  Monitor,
  Package,
  Palette,
  Rocket,
  Server,
  Sigma,
  Sparkles,
  Sprout,
  Target,
  Telescope,
  TrendingUp,
  User,
  Users,
  Workflow,
  Wrench,
  X,
} from 'lucide-react';

type IconComponent = ComponentType<SVGProps<SVGSVGElement>>;

/** Same frame and stroke settings Lucide uses, for the two hand-drawn glyphs. */
function strokeGlyph(children: ReactNode): IconComponent {
  return function Glyph(props: SVGProps<SVGSVGElement>) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={24}
        height={24}
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth={2}
        strokeLinecap="round"
        strokeLinejoin="round"
        {...props}
      >
        {children}
      </svg>
    );
  };
}

const Github = strokeGlyph(
  <>
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </>,
);

const Linkedin = strokeGlyph(
  <>
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
    <rect width="4" height="12" x="2" y="9" />
    <circle cx="4" cy="4" r="2" />
  </>,
);

const registry: Record<string, IconComponent> = {
  'lucide:arrow-right': ArrowRight,
  'lucide:at-sign': AtSign,
  'lucide:bot-message-square': BotMessageSquare,
  'lucide:brain': Brain,
  'lucide:briefcase': Briefcase,
  'lucide:check': Check,
  'lucide:chevron-down': ChevronDown,
  'lucide:clock': Clock,
  'lucide:cloud': Cloud,
  'lucide:code-xml': CodeXml,
  'lucide:download': Download,
  'lucide:file-text': FileText,
  'lucide:gamepad-2': Gamepad2,
  'lucide:gauge': Gauge,
  'lucide:github': Github,
  'lucide:globe': Globe,
  'lucide:graduation-cap': GraduationCap,
  'lucide:hammer': Hammer,
  'lucide:handshake': Handshake,
  'lucide:layers': Layers,
  'lucide:lightbulb': Lightbulb,
  'lucide:linkedin': Linkedin,
  'lucide:mail': Mail,
  'lucide:map-pin': MapPin,
  'lucide:menu': Menu,
  'lucide:monitor': Monitor,
  'lucide:package': Package,
  'lucide:palette': Palette,
  'lucide:rocket': Rocket,
  'lucide:server': Server,
  'lucide:sigma': Sigma,
  'lucide:sparkles': Sparkles,
  'lucide:sprout': Sprout,
  'lucide:target': Target,
  'lucide:telescope': Telescope,
  'lucide:trending-up': TrendingUp,
  'lucide:user': User,
  'lucide:users': Users,
  'lucide:workflow': Workflow,
  'lucide:wrench': Wrench,
  'lucide:x': X,
};

interface Props extends SVGProps<SVGSVGElement> {
  name: string;
}

export default function IconPart({ name, ...rest }: Props) {
  const Icon = registry[name];
  if (!Icon) {
    if (import.meta.env.DEV) console.warn(`IconPart: unknown icon "${name}"`);
    return null;
  }
  return <Icon aria-hidden="true" {...rest} />;
}
